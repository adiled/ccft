...
//! - [`classify_turns`] / [`classify_turns_prob_with_model`] — driver vs bot turns.
//! - [`fit_gap_mixture`] / [`GapMixture`] — 2-component log-normal EM on turn gaps.
//! - [`Baseline::from_records`] — the user fingerprint used to z-score signals.
...
impl Aggregate {
    ...
    /// Fold new records into an existing aggregate — O(new), so a live UI can
    /// keep the range aggregate current without re-reading the whole ledger
    /// on every tick. New records must be chronologically appended and already
...
/// Deterministic driver/bot classification: within a session, a turn is a
/// Bot when it follows the previous turn within `BOT_LOOP_THRESHOLD` seconds.
...
/// Fit the 2-component log-normal EM mixture over a session's turn gaps.
/// Returns `None` when there isn't enough data or one component collapses.
...
/// Probabilistic driver/bot classification (gap mixture + lexical wordology),
/// also returning the fitted model for inspection. Empty input → all Driver.
...
impl Baseline {
    pub fn from_records(records: &[Record]) -> Self {
        ...
        for recs in by_date_all.values() {
            if recs.len() < 5 {
                continue;
...
/// Driver score (0-100): how hard the human is driving vs their baseline.
...
/// Thinking-rot: heavy hidden reasoning + repetitive bot output.
...
/// Bot score (0-100): output health — brevity, stalling, wandering, cache,
/// thinking-rot, blended and shrunk by confidence.
